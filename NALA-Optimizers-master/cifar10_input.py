...
def _get_images_labels(batch_size, split, distords=False):
  ...
  dataset = tfds.load(name='cifar100', split=split)####cifar10####fashion_mnist
  scope = 'data_augmentation' if distords else 'input'##########distords=True#
...
class DataPreprocessor(object):
  ...
  def __call__(self, record):
    ...
    if self._distords:  # training
      ...
      img = tf.random_crop(img, [IMAGE_SIZE, IMAGE_SIZE, 3])###[IMAGE_SIZE, IMAGE_SIZE, 3]#
