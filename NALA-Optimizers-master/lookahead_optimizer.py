  ...
  def apply_gradients(self, grads_and_vars, global_step=None, name=None):
    ...
    def _update_variables():#################
      ...
      for (grad, var) in grads_and_vars: ########### grads_and_vars包含梯度值和参数值
        ...
        next_var = (1.0 - self._alpha) * local + self._alpha * var ##########外循环参数更新 slow weights = local + alpha*(var-local)
        global_assignments.append(state_ops.assign(var, next_var, use_locking=self._opt._use_locking))
        local_assignments.append(state_ops.assign(local, next_var, use_locking=self._opt._use_locking))#######更新外循环参数
    ...
    with ops.control_dependencies([local_update]):
      condition = math_ops.equal(math_ops.mod(self._get_step_accumulator(), self._interval_steps), 0)###判断是否更新外循环参数
